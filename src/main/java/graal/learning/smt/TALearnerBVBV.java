...
public class TALearnerBVBV extends TALearnerGeneric<BitvectorFormula, BitvectorFormula,BitvectorFormula>{
    ...
    public TALearnerBVBV(int nrStates, int nrTrans, int k_urgency, boolean bfsMode, int maxGuardValue, int bvSizeTimed, Set<Symbol> alphabet,
        int roundingFactor) {
    ...
    protected void initAdditional() {
        ...
        this.k_urgency_bv = bvMgr.makeBitvector(bvSizeTimed,k_urgency*roundingFactor);
    }
    ...
    protected BitvectorFormula makeGuardValue(Integer number) {
        return bvMgr.makeBitvector(bvSizeTimed,number*roundingFactor);
    }

    @Override
    protected BitvectorFormula makeTimeValue(Double number) {
        return bvMgr.makeBitvector(bvSizeTimed, (int)Math.round(number * roundingFactor));
    }

    protected BitvectorFormula makeDiscreteVar(String name) {
        return bvMgr.makeVariable(bvSizeDiscrete,name);
    }
    protected BitvectorFormula makeTimeVar(String name) {
        return bvMgr.makeVariable(bvSizeTimed,name);
    }
    ...
    @Override
    protected BitvectorFormula performDelay(BitvectorFormula curr_clock, BitvectorFormula delay) {
        return bvMgr.add(delay, curr_clock);
    }
    ...
    @Override
    protected BooleanFormula timeEqual(BitvectorFormula clockVar1, BitvectorFormula clockVar2) {
        return bvMgr.equal(clockVar1,clockVar2);
    }
    ...
    @Override
    protected BooleanFormula timeLessThan(BitvectorFormula clock, BitvectorFormula guard) {
        return bvMgr.lessThan(clock,guard,useSigned);
    }

    @Override
    protected BooleanFormula timeLessOrEqual(BitvectorFormula clock, BitvectorFormula guard) {
        return bvMgr.lessOrEquals(clock,guard,useSigned);
    }

    @Override
    protected BooleanFormula timeGreaterOrEqual(BitvectorFormula clock, BitvectorFormula guard) {
        return bvMgr.greaterOrEquals(clock,guard,useSigned);
    }
...
//            for (int s= 0; s< nrStates;s++) {
//                System.out.println(s + ": c <= " + model.evaluate(fmgr.callUF(invariant, bvMgr.makeBitvector(bvSizeDiscrete,s))));
//                dot_string.append(String.format("%s [shape=\"circle\" label=\"c <= %s\"];\n",s,
//                        model.evaluate(fmgr.callUF(invariant, bvMgr.makeBitvector(bvSizeDiscrete,s))).toString()));
//                for (int t=0; t<nrTrans;t++)
//                {
//                    if (model.evaluate(fmgr.callUF(source, bvMgr.makeBitvector(bvSizeDiscrete,t))).intValue()==s)
//                    {
//
//                        String t_label=getKeyByValue(discreteToInt,model.evaluate(fmgr.callUF(label, bvMgr.makeBitvector(bvSizeDiscrete,t))).intValue()).getSymbol();
//                        String reset_str = model.evaluate(fmgr.callUF(reset, bvMgr.makeBitvector(bvSizeDiscrete,t))).booleanValue() ? "true" : "false";
//
//
//                        dot_string.append(String.format("%s -> %s [label=\"%s,%s <= c <= %s,res=%s\"];\n",
//                                model.evaluate(fmgr.callUF(source, bvMgr.makeBitvector(bvSizeDiscrete,t))),
//                                model.evaluate(fmgr.callUF(target, bvMgr.makeBitvector(bvSizeDiscrete,t))),
//                                t_label,
//                                model.evaluate(fmgr.callUF(guard_below, bvMgr.makeBitvector(bvSizeDiscrete,t))),
//                                model.evaluate(fmgr.callUF(guard_above, bvMgr.makeBitvector(bvSizeDiscrete,t))),
//                                reset_str));
//                        System.out.printf("trans %s: %s -> %s [label=\"%s,%s <= c <= %s,res=%s\"];\n%n",t,
//                                model.evaluate(fmgr.callUF(source, bvMgr.makeBitvector(bvSizeDiscrete,t))),
//                                model.evaluate(fmgr.callUF(target, bvMgr.makeBitvector(bvSizeDiscrete,t))),
//                                t_label,
//                                model.evaluate(fmgr.callUF(guard_below, bvMgr.makeBitvector(bvSizeDiscrete,t))),
